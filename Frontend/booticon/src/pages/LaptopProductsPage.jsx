...
function LaptopProductsPage() {
    ...
    useEffect(() => {
        // Fetch products
        let url = 'https://localhost:7023/api/Category/1';
        if (selectedBrand) {
            url = `https://localhost:7023/api/Filter/FilterForCategoryAndBrand?categoryId=1&brandId=${selectedBrand}`;
        }
        axios.get(url)
            .then(response => {
